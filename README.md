...
`reply_mode` 同时控制普通回复和任务运行中/完成回复：`text` 是纯文本气泡，`post` 是飞书富文本消息，`card` 是卡片消息。`post` 模式下任务回复只发送最终富文本结果，避免飞书拒绝更新富文本消息。
...
- `text` / `card` 模式下，执行请求会先回复运行态消息，再更新为 `已接收 / 排队中 / 处理中 / 已完成 / 失败`
- `post` 模式下，执行请求只发送最终富文本结果，不更新中间状态消息
