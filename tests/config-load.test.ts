...
describe('config load', () => {
  it('merges global and project config layers', async () => {
    ...
    const repo = await fs.mkdtemp(path.join(os.tmpdir(), 'feique-repo-'));
    ...
    await fs.mkdir(path.join(repo, '.feique'), { recursive: true });
    await fs.writeFile(
      path.join(repo, '.feique', 'config.toml'),
      [
        '[service]',
  ...
  it('defaults run_timeout_ms to 30 minutes and seeds init templates with the same value', async () => {
    ...
    const configPath = path.join(home, '.feique', 'config.toml');
