...
describe('instance lock', () => {
  it('creates and releases a lock file', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'feique-lock-'));
    tempDirs.push(dir);
  ...
  it('rejects when another live process owns the lock', async () => {
    ...
    const lockPath = path.join(dir, 'feique.lock');
    await fs.writeFile(
      lockPath,
      JSON.stringify({
    ...
    ).rejects.toThrow('Another feique instance is already running');
  });
